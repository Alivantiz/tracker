...
export default function TotalsView({ date }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: sales }, { data: expenses }, { data: purchases }, { data: salaries }] = await Promise.all([
        supabase.from('sales').select('*').eq('day_id', day.id),
        supabase.from('expenses').select('*').eq('day_id', day.id),
        supabase.from('purchases').select('*').eq('day_id', day.id),
        supabase.from('salaries').select('*').eq('day_id', day.id),
      ])
      setData({ sales: sales||[], expenses: expenses||[], purchases: purchases||[], salaries: salaries||[] })
      setLoading(false)
    }
    load()
  }, [d])
  ...
  const stats = data ? calcDayStats(data.sales, data.expenses, data.purchases, data.salaries) : null
        ...
        {!stats ? (
          ...
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
              ...
              <Tile label="Чистые продажи" value={`${stats.net} шт`} color="var(--green)" border="rgba(76,175,125,0.3)" bg="rgba(76,175,125,0.06)" fullWidth={activePayments.length === 0} />
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns: activePayments.length === 1 ? '1fr' : '1fr 1fr', gap:8, marginBottom:8 }}>
              ...
              {activePayments.length === 1 && (
                <Tile label="Итого выручка" value={fmtMoney(stats.revenue)} color="var(--accent)" border="var(--accent)" bg="rgba(245,166,35,0.08)" />
              )}
              {activePayments.length === 0 && (
                <Tile label="Итого выручка" value={fmtMoney(0)} color="var(--muted)" fullWidth />
              )}
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
              <Tile label="📦 Закупы"   value={fmtMoney(stats.totalPurchases)} color="var(--purple)" border="rgba(165,123,245,0.3)" bg="rgba(165,123,245,0.06)" />
              <Tile label="💸 Расходы"  value={fmtMoney(stats.totalExpenses)}  color="var(--orange)" border="rgba(245,131,74,0.3)"  bg="rgba(245,131,74,0.06)"  />
              <Tile label="👷 Зарплата" value={fmtMoney(stats.totalSalaries)}  color="var(--blue)"   border="rgba(91,138,245,0.3)"  bg="rgba(91,138,245,0.06)"  fullWidth />
            </div>
            ...
            {data.purchases.length > 0 && <DetailList title="📦 Закупы"   items={data.purchases} color="var(--purple)" />}
            {data.expenses.length  > 0 && <DetailList title="💸 Расходы"  items={data.expenses}  color="var(--orange)" />}
            {data.salaries.length  > 0 && <DetailList title="👷 Зарплата" items={data.salaries}  color="var(--blue)"   />}
          </>
        )}
      </div>
    </div>
...
function Tile({ label, value, color, border, bg, fullWidth }) {
  return (
    <div style={{
      background: bg || 'var(--bg2)', border:`1px solid ${border || 'var(--border)'}`,
      borderRadius:10, padding:'10px 12px', gridColumn: fullWidth ? '1/-1' : undefined
    }}>
