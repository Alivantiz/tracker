...
export default function SettingsView() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: sh }, { data: cats }, { data: mats }] = await Promise.all([
    ...
    <div className="page fade-in">
      ...
      <div style={{ padding:'16px 16px 0' }}>
            ...
            {editShop?.id === s.id ? (
              <div>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  <Btn color="var(--green)" onClick={saveShopEdit}>Сохранить</Btn>
                  <Btn onClick={() => setEditShop(null)}>Отмена</Btn>
                </div>
              </div>
            ) : (
        ...
        <div style={{ fontSize:12, color:'var(--muted)', marginBottom:10 }}>
          Укажи цену за единицу — при добавлении расхода просто выбираешь материал и количество
        </div>
        {materials.map(m => (
          <div className="card" key={m.id}>
            {editMat?.id === m.id ? (
              <div>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  <Btn color="var(--green)" onClick={saveMatEdit}>Сохранить</Btn>
                  <Btn onClick={() => setEditMat(null)}>Отмена</Btn>
                </div>
              </div>
            ) : (
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        ...
        <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 16px', fontSize:13, color:'var(--muted)', lineHeight:1.6, marginBottom:16 }}>
          ...
          <span style={{ fontSize:11 }}>v3.2.0</span>
        </div>
      </div>
    </div>
  )
