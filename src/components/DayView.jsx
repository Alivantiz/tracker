...
export default function DayView({ onDateChange }) {
  ...
  async function addMaterialExpense(matId, qty) {
    ...
    const category = mat.category || '💸 Расходы'
    const { data } = await supabase.from('expenses').insert({ day_id: dayId, name, amount, category }).select().single()
  ...
  const totalPurchases = expenses.filter(e => (e.category||'💸 Расходы') === '📦 Закупы').reduce((a,e) => a+(e.amount||0), 0)
  const totalExpenses2 = expenses.filter(e => (e.category||'💸 Расходы') === '💸 Расходы').reduce((a,e) => a+(e.amount||0), 0)
  const totalSalaries  = expenses.filter(e => (e.category||'💸 Расходы') === '👷 Зарплата').reduce((a,e) => a+(e.amount||0), 0)
  ...
  const TABS = [
    ...
    { key:'expenses', label:`💸 Расходы${expCount > 0 ? ` (${expCount})` : ''}` },
  ]
    ...
    <div className="page fade-in">
      ...
      {tab === 'expenses' && (
        <div style={{ padding:'12px 12px 0' }}>
          ...
          {['📦 Закупы','💸 Расходы','👷 Зарплата'].map(cat => {
            const catExpenses = expenses.filter(e => (e.category||'💸 Расходы') === cat)
            if (catExpenses.length === 0) return null
            const catColor = cat==='📦 Закупы'?'var(--purple)':cat==='👷 Зарплата'?'var(--blue)':'var(--orange)'
            const catTotal = catExpenses.reduce((a,e) => a+(e.amount||0), 0)
            return (
              <div key={cat} style={{ marginBottom:8 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                  <div style={{ fontSize:9, color:catColor, textTransform:'uppercase', letterSpacing:.8, fontWeight:700 }}>{cat}</div>
                  <div style={{ fontSize:11, color:catColor, fontWeight:700 }}>{fmtMoney(catTotal)}</div>
                </div>
                {catExpenses.map(e => (
                  <div className="card" key={e.id} style={{ borderColor:catColor+'40', background:catColor+'08', marginBottom:6 }}>
                    <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                      <input className="input" type="text" value={e.name||''} placeholder="Описание"
          ...
          {expenses.length > 0 && (
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:10, padding:'10px 14px', marginTop:4 }}>
              {totalPurchases > 0 && <Row label="📦 Закупы"   value={fmtMoney(totalPurchases)} valColor="var(--purple)" />}
              {totalExpenses2 > 0 && <Row label="💸 Расходы"  value={fmtMoney(totalExpenses2)} valColor="var(--orange)" />}
              {totalSalaries  > 0 && <Row label="👷 Зарплата" value={fmtMoney(totalSalaries)}  valColor="var(--blue)" />}
              <div style={{ borderTop:'1px solid var(--border)', marginTop:6, paddingTop:6 }}>
                <Row label="Итого" value={fmtMoney(stats.totalExpenses)} valColor="var(--red)" bold size={14} />
              </div>
            </div>
          )}
        </div>
...
function MaterialPicker({ materials, onAdd }) {
  ...
  return (
    <div style={{ background:'rgba(245,131,74,0.04)', border:'1px solid rgba(245,131,74,0.2)', borderRadius:10, padding:10, marginBottom:8 }}>
      <div style={{ fontSize:10, color:'var(--orange)', textTransform:'uppercase', letterSpacing:.8, fontWeight:700, marginBottom:7 }}>Из справочника</div>
